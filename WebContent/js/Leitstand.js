	 ...
	 $('#btnSetSnoozeAndDescription').unbind('click').click(function() {
		 writeSnoozeAndDescriptionInDB();
  	   	 ...
  	   	 $.ajax({
  	   	   	 ...
  	   	   	 url: host+'/WebContent/Leitstand/services/MobileNotification/setSnoozeAndMessage',
  	   	   	 dataType: "json",
  	   	   	 data: JSON.stringify({
  	   	   		 "id": id,
  	   	   		 "snooze_time": snooze,
    	 ...
    	 function  setTaskSolvedInDB(){
    		...
    		if(confirm("Wollen Sie dieses Problem wirklich als behoben markieren?")){
    			...
    			var id = document.getElementById("process_id").value;
    	    	 ...
    	    	 type: 'POST',
    	    	 contentType: 'application/json',
    	    	 url: host+'/WebContent/Leitstand/services/MobileNotification/solveTask',
    	    	 dataType: "json",
    	    	 data: JSON.stringify({
      	   	   		 "id": id,
    	    		 "state": state,
        	 ...
        	 function  sendTestmailToServer(){
        		...
        		if(confirm("Wollen Sie wirklich eine Testmail versenden?")){
        			...
        			var id = 123;
        	    	 ...
        	    	 type: 'POST',
        	    	 contentType: 'application/json',
        	    	 url: host+'/WebContent/Leitstand/services/MobileNotification/sendTestmail',
        	    	 dataType: "json",
        	    	 data: JSON.stringify({
          	   	   		 "id": id,
        	    		 "state": state,
